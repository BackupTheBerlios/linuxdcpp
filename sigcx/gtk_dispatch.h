...
*/
#if !defined(__LIB_SIGCX_GTK_DISPATCH_H)
#define __LIB_SIGCX_GTK_DISPATCH_H
 ...
 */
class GtkDispatcher : public SignalDispatcher
{
    struct FileEvent {
        GtkDispatcher *disp;
        guint tag;
	...
	FileEvent() {}
	FileEvent(GtkDispatcher *_disp, 
                  guint _tag, const Handler& _cb, Event _ev)
	...
	TimerEvent () {}
	TimerEvent (GtkDispatcher *_disp, guint _tag, const Handler& _cb)
	    : disp (_disp), tag (_tag), cb (_cb)
          {}
    };
    typedef std::map<HandlerID, FileEvent> FMap;
    typedef std::map<HandlerID, TimerEvent> TMap;
  ...
  public:
    ...
    virtual HandlerID add_timeout_handler(const Handler& h, 
                                          const TimeVal& tv);
