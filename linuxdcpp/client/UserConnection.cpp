...
void UserConnection::on(BufferedSocketListener::Line, const string& aLine) throw () {
	...
	if(cmd == "$MyNick") {
		if(!param.empty())
			fire(UserConnectionListener::MyNick(), this, Text::convert(param, encoding, "UTF-8"));
		...
		x = param.find('$');
		if(x != string::npos) {
			fire(UserConnectionListener::Get(), this, Text::convert(param.substr(0, x), encoding, "UTF-8"), Util::toInt64(param.substr(x+1)) - (int64_t)1);
		}
	} else if(cmd == "$Key") {
		if(!param.empty())
