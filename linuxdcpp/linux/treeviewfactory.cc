...
TreeView::setView(GtkTreeView *view, bool padding, SettingsManager::StrSetting orderSetting, SettingsManager::StrSetting widthSetting)
{
	this->view = view;
...
void TreeView::insertColumn(const string &title, const int id, const GType gtype, const TreeViewFactory::type_t type, const int width, const int id_pixbuf)
{
...
void TreeView::getColumn(gint c, std::vector<std::string> *l) {
	GtkTreeModel *m = gtk_tree_view_get_model(view);
	GtkTreeIter it;
